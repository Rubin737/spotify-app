...
function Table({ className, ...props }) {
    return (_jsx("div", { "data-slot": "table-container", className: "relative w-full overflow-x-auto", children: _jsx("table", { "data-slot": "table", className: cn("w-full caption-bottom text-sm rounded-md", className), ...props }) }));
}
...
function TableRow({ className, ...props }) {
    return (_jsx("tr", { "data-slot": "table-row", className: cn("border-b  border-b-gray-700 transition-colors  hover:bg-zinc-700/50! has-aria-expanded:bg-muted/50 data-[state=selected]:bg-muted", className), ...props }));
}
...
function TableCell({ className, ...props }) {
    return (_jsx("td", { "data-slot": "table-cell", className: cn(" p-1 align-middle whitespace-nowrap [&:has([role=checkbox])]:pr-0", className), ...props }));
}
